...
from services import get_conversation_history, get_creative_claude_response, get_standard_claude_response
...
def load_json_view(file_name: str) -> Dict:
    """Load JSON view file."""
...
def handle_save_settings(ack, body, logger):
...
@app.event("message")
def handle_message(message, say, client, ack):
    ...
    logger.info(f"Received message from user {user} in channel {channel} with id {message['client_msg_id']}")
